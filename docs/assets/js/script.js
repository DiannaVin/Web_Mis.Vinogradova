...
// Глобальная переменная для отслеживания загрузки DOM
let domLoaded = false;
...
// Функция для безопасного получения всех элементов
...
function updateYear() {
    ...
    if (yearElement) {
        ...
        yearElement.innerText = yearDate;
    }
}
...
// Обработка плавной прокрутки для якорных ссылок
...
function initLinkAnimations() {
    safeQuerySelectorAll('.link-item').forEach(link => {
...
// Анимация фоновых фигур при прокрутке
function initScrollAnimations() {
    window.addEventListener('scroll', function() {
        const scrolled = window.pageYOffset;
        const shapes = safeQuerySelectorAll('.shape');
...
// Инициализация печатной машинки для био
function initTypewriter() {
    setTimeout(() => {
        const bioElement = safeQuerySelector('.bio');
        if (bioElement) {
...
// Инициализация конфетти для ссылок
...
function createFloatingCuteElements() {
    const cuteElements = ['💕', '💖', '🌸', '✨', '💫', '🌺', '🌷'];
    const container = safeQuerySelector('.background-animation');
...
// Добавляем CSS для плавающих элементов
...
function initTagAnimations() {
    safeQuerySelectorAll('.tag').forEach(tag => {
...
function addHeartBeatToAvatar() {
    const avatar = safeQuerySelector('.avatar');
    if (avatar) {
...
function addWaveToFooterIcons() {
    const footerIcons = safeQuerySelectorAll('.footer-icon');
...
function addSparkleToName() {
    const name = safeQuerySelector('.name');
    if (name) {
...
function initIntersectionObserver() {
    ...
    safeQuerySelectorAll('.link-item, .tag').forEach(el => {
        el.style.opacity = '0';
...
document.addEventListener('DOMContentLoaded', function() {
    if (domLoaded) return; // Предотвращаем повторную инициализацию
    domLoaded = true;
    
    console.log("🚀 Initializing TapLink...");
    ...
    // Дополнительные эффекты с задержкой
    setTimeout(() => {
        addHeartBeatToAvatar();
        addWaveToFooterIcons();
        addSparkleToName();
        createCuteParticleEffect();
        createGlitter();
        setTimeout(createJumpingCats, 1000);
    }, 1000);
